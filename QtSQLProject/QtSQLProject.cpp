...
// Функция обработчик глобальных событий
//############################################

/*bool QtSQLProject::eventFilter(QObject* watched,QEvent* event)
{
...
void QtSQLProject::clear_form() // Функция очистки полей ввода
{
...
void QtSQLProject::seach_email() // Функция поиска по почтовому адресу
{
	query = QSqlQuery(db);
...
void QtSQLProject::seach_phone() // Функция поиска по номеру телефона
{
	query = QSqlQuery(db);
...
void QtSQLProject::seach_id()// Функция поиска по ID
{
	query = QSqlQuery(db);
...
{
	// id_seach = "SELECT * FROM addressbook WHERE ID = " + ui.dropID->text();
...
void QtSQLProject::show_table()
{
	...
	//view.setStyleSheet("background: grey"); //-- устанавливает серый цвет результата запроса
